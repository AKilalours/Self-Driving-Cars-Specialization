...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        if self._state == FOLLOW_LANE:
            ...
            # Check further ahead for stop signs (use extended lookahead)
            extended_index = min(goal_index + 5, len(waypoints) - 1)
            goal_index, stop_sign_found = self.check_for_stop_signs(waypoints, closest_index, extended_index)
            ...
            if self._stop_count == STOP_COUNTS:
                closest_len, closest_index = get_closest_index(waypoints, ego_state)
                goal_index = self.get_goal_index(waypoints, ego_state, closest_len, closest_index)
                # Look further ahead to get past the stop sign
                goal_index_lookahead = min(goal_index + 10, len(waypoints) - 1)
                _, stop_sign_found = self.check_for_stop_signs(waypoints, closest_index, goal_index_lookahead)
                self._goal_index = goal_index
                self._goal_state = waypoints[goal_index]
                if not stop_sign_found:
                    self._state = FOLLOW_LANE
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            arc_length += np.linalg.norm([
                waypoints[wp_index+1][0] - waypoints[wp_index][0],
                waypoints[wp_index+1][1] - waypoints[wp_index][1]
            ])
            wp_index += 1
            if arc_length >= self._lookahead:
                break
    ...
    def check_for_stop_signs(self, waypoints, closest_index, goal_index):
        for i in range(closest_index, min(goal_index + 2, len(waypoints) - 1)):
